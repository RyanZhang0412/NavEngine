"""CRC16-XMODEM table lookup (from Motion-HMICoreLib crc16.c)."""
...
def calc_crc16(data: bytes, init: int = 0) -> int:
    """Calculate CRC16-XMODEM over data (calc_crc16_by_table)."""
    crc_value = init
    for byte in data:
        crc_value = ((crc_value << 8) & 0xFFFF) ^ CRC16_TABLE[(crc_value >> 8) ^ byte]
    return crc_value


def calc_crc16_continue(data: bytes, last_value: int) -> int:
    """Continue CRC16 calculation from last_value (calc_crc16_by_table_continue)."""
    return calc_crc16(data, init=last_value)
